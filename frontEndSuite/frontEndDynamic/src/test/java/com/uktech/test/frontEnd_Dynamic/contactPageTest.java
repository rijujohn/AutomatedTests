...
import com.uktech.page.frontEndDynamic.contactPageRepo;
import com.uktech.helpers.extent.ExtentManager;
		...
		for(int i = 1;i<intRCount;i++)
		{
			...
			if (strExecute.toUpperCase().equals("YES"))
			{
				...
				{	
					...
					String strTemplate = mapData.get(i+"Template");
					...
					implicitWait(waitTime);
					//Contact page tests
					ContactPage.verifyContactPageHeader(strTemplate);
					ContactPage.verifyAddress(strTemplate,strCustomerName, strDoorNumber, strStreet, strTown,strCity,strPostCode);
					ContactPage.verifyOpenHours(strTemplate,arrOpenHourList);
					ContactPage.verifyContactTextInfo(strTemplate,strTakeAwayNumber,strContactName,strContactNumber);
